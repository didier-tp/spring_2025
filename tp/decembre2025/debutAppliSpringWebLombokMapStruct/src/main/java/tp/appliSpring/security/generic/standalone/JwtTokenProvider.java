...
public class JwtTokenProvider {
    ...
    public String generateToken(Authentication authentication) {
        ...
        List<String> roleNameList=MySecurityUtil.roleNameList(authentication);
		...
		if(rolesInClaim!=null){
			...
			if(rolesInClaimAsString.length()>2){
		        ...
		        for(String roleName : tabOfRoleNames){
		        	...
		        	if(!(springSecurityRoleName.startsWith(MySecurityUtil.DEFAULT_SPRING_SECURITY_ROLE_PREFIX))){
		        		springSecurityRoleName=MySecurityUtil.DEFAULT_SPRING_SECURITY_ROLE_PREFIX + roleName;
		        	}
